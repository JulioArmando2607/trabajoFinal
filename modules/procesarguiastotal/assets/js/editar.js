...
$(document).ready(function () {
    ...
    if ($("#remitente").val() != null) {
        $.ajax({
            type: "POST",
            dataType: 'json',
            url: APP_URL + '/procesarguiastotal/default/buscar-direccion',
    ...
    if ($("#id_guia").val() != null) {
        $.ajax({
            type: "POST",
            dataType: 'json',
            url: APP_URL + '/procesarguiastotal/default/detalle-guia',
            contentType: 'application/x-www-form-urlencoded; charset=UTF-8',
            data: {
                id_guia: $("#id_guia").val()
            },
    ...
    if ($("#id_guia").val() != null) {
        $.ajax({
            type: "POST",
            dataType: 'json',
            url: APP_URL + '/procesarguiastotal/default/guia-cliente',
            contentType: 'application/x-www-form-urlencoded; charset=UTF-8',
            data: {
                id_guia: $("#id_guia").val()
            },
        ...
        $("#frm-guia-remision").validate({
            ...
            submitHandler: function () {
                ...
                $.ajax({
                    type: "POST",
                    dataType: 'json',
                    url: APP_URL + '/procesarguiastotal/default/update',
                    contentType: 'application/x-www-form-urlencoded; charset=UTF-8',
                    data: {
                        id_guia: $("#id_guia").val(),
                        //  serie: serie,
                    ...
                    success: function (response) {
                        window.location.href = APP_URL + '/procesarguiastotal';
                    }
                });
            }
        });
...
function mostrarTablaTemporalEdit() {
    ...
    detalle_guia_edit.forEach(item => {
        temporalDetalleGuia += '<tr><td>' + item.producto + '</td><td>' + item.descripcion + '</td><td>' + item.unidad + '</td><td>' + item.cantidad + '</td><td>' + item.peso + '</td><td>'+ item.largo +'</td><td>'+ item.ancho + '</td><td>'+ item.alto + '</td><td>' + item.volumen + '</td><td><i class="flaticon-delete text-danger bg-hover-light" onclick="eliminarTemporalDetalleGuiaEdit(' + item.identificadorDetalle + ')"></i></td></tr>';
    });
