...
namespace app\modules\guiaremision\query;
...
class Consultas {
    ...
    public static function getExExcel($fecha)
    {

        $command = Yii::$app->db->createCommand('call totalGuiasMes(:fechal)');
        $command->bindValue(':fechal', $fecha);
        $result = $command->queryAll();
    ...
    public static function exportarDirecciones() {
        $command = Yii::$app->db->createCommand('call exportarDirecciones()');
        $result = $command->queryAll();
        return $result;
    }
    public static function exportarEntidades() {
        $command = Yii::$app->db->createCommand('call exportarEntidades()');
        $result = $command->queryAll();
        return $result;
    }
}
