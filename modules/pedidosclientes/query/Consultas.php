...
namespace app\modules\pedidosclientes\query;
...
class Consultas {
    ...
    public static function getImprimirExcel() {
     ...
     public static function getDatosCorreoAct($numero_solicitud)
    {
        $result = [];
        try {
            $command = Yii::$app->db->createCommand('call DatosCorreoGuiaP(:numero_solicitud)');
            $command->bindValue(':numero_solicitud', $numero_solicitud);
            $result = $command->queryAll();
        } catch (\Exception $e) {
